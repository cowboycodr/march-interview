...
export default function Posts() {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            const userIds = Array.from(new Set(posts.map(post => post.userId).filter(id => id !== 0)));
    ...
    const getUserDisplay = (userId: number) => {
        ...
        return user ? `@${user.username.toLowerCase()}` : `User ${userId}`;
    }
                    ...
                    {editingId === post.id ? (
                        ...
                        <>
                            ...
                            <div className="flex justify-between">
                                ...
                                <div className="space-x-2">
                                    <button onClick={() => handleEdit(post)}><Pencil size={16} /></button>
                                    <button onClick={() => handleDelete(post.id)}><Trash2 size={16} /></button>
                                </div>
                            </div>
                        </>
                    )}
